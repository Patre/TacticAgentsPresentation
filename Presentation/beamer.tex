...
\begin{columns}[c]
	\begin{column}[c]{0.48\linewidth}
		\begin{centering}
		\includegraphics[trim=0cm 0cm 0cm 0cm, clip=true, width=0.2\paperwidth]{../ressources/Roman_soldier}
		\end{centering}
	\end{column}
	\begin{column}[c]{0.48\linewidth}
		\begin{itemize}
...
\begin{frame}{Hiérarchie}
\makebox[0.99\linewidth]{
\begin{tikzpicture}[	level 1/.style={sibling distance=60mm}, 
				level 2/.style={sibling distance=25mm}, 
				level 3/.style={sibling distance=20mm}]
	\node [circle,draw] (z){général}
		child {node [item,draw] (a) {officier}
			child {node [item,draw] (b) {sous-officier}
					child {node [item,draw] (d) {militaire}}
					child {node [item,draw] (e) {militaire}}
			}
			child {node [item,draw] (c) {sous-officier}}
		}
		child {node [item,draw] (j) {officier}
			child {node [item,draw] (k) {sous-officier}}
			child {node [item,draw] (l) {sous-officier}
				child {node [item,draw] (o) {militaire}}
				child {node [item,draw] (p) {militaire}}
			}
		};
	\path (a) -- (j)  node [midway] {\ldots};
	\path (b) -- (l) node [midway] {\ldots};
	\path (d) -- (p) node [midway] {\ldots};
\end{tikzpicture}
}
\footlineextra{}
\end{frame}
