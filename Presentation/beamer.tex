...
\begin{tikzpicture}[remember picture,overlay]
	...
	\node[draw,item,anchor=north east] at($(current page.north east)+(-0.2cm,-5.7cm)$) {contre une position défensive};
	\node[draw,item,anchor=south east] at($(current page.south east)+(-0.2cm,2.5cm)$) {prise en tenaille};
	\node[draw,item,anchor=north west] at($(current page.north west)+(0.4cm,-2.0cm)$) {embuscade};
\end{tikzpicture}
