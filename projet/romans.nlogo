;; DECLARATIONS
breed [ formations formation ]
formations-own [  ]
breed [ hordes horde ]
hordes-own [ ]

turtles-own [ speed go-method state-method ]
...
to global-setup
  ...
  ask patches
  [
    ;; set colour also in setup (or it will be black)
    set pcolor green
  ]
  
  ;; create formations
  create-formations number-formations
  [
    ; position and speed
...
to global-go
  ;; update entities
  ask turtles [ run go-method ]
  ;ask patches [ patch-go ]
  ;; timing & plotting
  tick
  global-update-plot
end

to global-update-plot
